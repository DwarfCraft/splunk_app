...
class DeleteDevice(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def post(self, request):
        ...
        device_owner = user
        if 'device_owner' in request['query'] and isinstance(request['query']['device_owner'], unicode):
        ...
        return delete_device(user, device_owner, device_key, system_authtoken, user_authtoken)
...
def delete_device(user, device_owner, device_key, system_authtoken, user_authtoken):
    ...
    LOGGER.info('device key=%s (of device_owner=%s) deleted from spacebridge by user=%s' % (device_key, device_owner, user))
