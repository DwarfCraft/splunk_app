...
TIMEOUT_SEC = cloudgateway_config.get_async_timeout_secs(default='5')
BATCH_UPLOAD_DOC_LIMIT = 1000


class AsyncKvStoreClient(AsyncClient):
    """
    ...
    def async_kvstore_post_or_update_request(self, collection, data, auth_header, key_id=None, owner=NOBODY,
                                             params=None):
        """
        ...
        """
        uri = self.get_kvstore_uri(owner, collection, key_id)
        response = yield self.async_post_request(uri=uri, data=data, params=params, auth_header=auth_header,
                                                 timeout=TIMEOUT_SEC)
        if response.code == http.NOT_FOUND and key_id is not None:
    ...
    @defer.inlineCallbacks
    def async_batch_save_request(self, auth_header, collection, entries, owner=NOBODY,
        ...
        while start_entry < len(entries):
            batch_upload_response = yield self.async_post_request(
                uri=uri, auth_header=auth_header, data=json.dumps(entries[start_entry:end_entry]))
            if batch_upload_response.code != http.OK:
                message = yield batch_upload_response.text()
            ...
            ids_from_this_batch = yield batch_upload_response.json()
            affected_ids.extend(ids_from_this_batch)

            start_entry += batch_size
            end_entry += batch_size

        defer.returnValue(affected_ids)
