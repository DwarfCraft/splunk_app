...
def generate_search_hash(dashboard_id,
    ...
    :param roles:  The roles of the user creating the search, used to allow that searches can be reused between users
                   with the same permission levels.
    ...
    if roles is not None:
        for role in sorted(roles):
            roles_string = "{}/{}".format(roles_string, role)

    search_pairs = ['dashboard_id', dashboard_id, 'search_id', search_id,
                    'input_token_string', input_token_string, 'roles_string', roles_string,
    ...
    return hashlib.sha256(search_string).hexdigest()
