...
from spacebridgeapp.drone_mode.data.enums import TVEventType, IPadEventType, is_valid_enum
...
class Subscription(SpacebridgeAppBase):
    ...
    def from_json(cls, json_obj):
        ...
        if json_obj:
            ...
            subscription.last_update_id = json_obj.get(cls.LAST_UPDATE_ID)
...
class SearchUpdate(SpacebridgeAppBase):
    ...
    def __repr__(self):
        ...
        :return:
        """
        return "key={}, post_search={}".format(self._key, self.post_search)
...
class DroneModeTVEvent(SpacebridgeAppBase):
    """Drone Mode TV Event"""

    def __init__(self,
                 data_object=None,
                 event_type=None):
        if not is_valid_enum(TVEventType, event_type):
            raise ValueError('Invalid Drone Mode TV Event Type={}'.format(event_type))

        if not (event_type and data_object):
            raise ValueError('Cannot initialize DroneModeTVEvent without a valid type or data')

        self.data_object = data_object
        self.event_type = event_type

    def set_protobuf(self, proto):
        """Takes a proto of type DroneModeTVEvent and populates
         the fields with the corresponding class values

        Arguments:
            proto {DroneModeTVEvent}
        """
        if self.data_object is not None:
            if self.event_type == TVEventType.TV_CONFIG:
                proto.tvConfig.CopyFrom(self.data_object.to_protobuf())
            elif self.event_type == TVEventType.MPC_BROADCAST:
                proto.startMPCBroadcast.CopyFrom(self.data_object.to_protobuf())
            elif self.event_type == TVEventType.TV_INTERACTION:
                proto.tvInteraction.CopyFrom(self.data_object.to_protobuf())

        else:
            raise ValueError('No data to convert to protobuf')
    ...
    def to_protobuf(self):
        ...
        Returns:
            proto {DroneModeTVEvent}
        """
        proto = subscription_pb2.DroneModeTVEvent()
        self.set_protobuf(proto)
        return proto

class DroneModeiPadEvent(SpacebridgeAppBase):
    """Drone Mode iPad Event"""

    def __init__(self,
                 data_object=None,
                 event_type=None):
        if not is_valid_enum(IPadEventType, event_type):
            raise ValueError('Invalid Drone Mode iPad Event Type={}'.format(event_type))

        if not (event_type and data_object):
            raise ValueError('Cannot initialize DroneModeiPadEvent without a valid type or data')

        self.data_object = data_object
        self.event_type = event_type

    def set_protobuf(self, proto):
        """Takes a proto of type DroneModeiPadEvent and populates
         the fields with the corresponding class values

        Arguments:
            proto {DroneModeiPadEvent}
        """
        if self.data_object is not None:
            if self.event_type == IPadEventType.TV_LIST:
                proto.nowPlayingTvs.CopyFrom(self.data_object.to_protobuf())
        else:
            raise ValueError('No data to convert to protobuf')
    ...
    def to_protobuf(self):
        ...
        Returns:
            proto {DroneModeIpadEvent}
        """
        proto = subscription_pb2.DroneModeiPadEvent()
        self.set_protobuf(proto)
        return proto
