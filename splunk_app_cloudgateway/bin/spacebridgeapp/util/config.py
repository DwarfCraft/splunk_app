...
def parse_proxy_settings(proxy_url, default_port=DEFAULT_HTTP_PORT):
    ...
    if auth is not None:
        auth = py23.b64encode_to_str(auth).strip()
    else:
        auth = None
...
class CloudgatewayConfig(object):
    """
    Class to encapsulate configuration settings for cloud gateway configuration.
    """
    ...
    # The amount of time in seconds before subscription_modular_input is restarted
    ...
    def get_cloud_gateway_id(self):
        """
        Helper to get the cloud_gateway_id, strip end whitespaces,
        replace inside whitespace with underscore and trim to 32 characters by default
        :return:
        """
        return self.format_cloud_gateway_id(self.get_config(key=self.CLOUD_GATEWAY_ID, default=""))

    @staticmethod
    def format_cloud_gateway_id(name):
        """
        Helper to get the cloud_gateway_id, strip leading/trailing whitespaces
        and trim to 64 characters by default
        :return:
        """
        formatted = name.strip()
        return formatted[:64].strip()  # removing any trailing whitespaces after truncation
    ...
    def get_proxy_cfg(self):
        try:
            # Initially look for proxyConfg in cloudgateway.conf
...
cloudgateway_config = CloudgatewayConfig(SPACEBRIDGE_APP_NAME, 'cloudgateway')
