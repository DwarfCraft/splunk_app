...
MESSAGE_VALUES = {
    http.CONFLICT: 'ERROR_ALREADY_REGISTERED',
    422: 'ERROR_APP_NOT_ENABLED',
    http.CREATED: 'SUCCESS'
}
...
class BasicAuthHeader(object):
    ...
    def validate(self, async_splunk_client, logger, request_context):
        """
        Validate the username, password against splunk's login api
        """


        response = yield async_splunk_client.async_get_splunk_cookie(self.username, self.password)
...
class JWTAuthHeader(object):
    ...
    def validate(self, async_splunk_client, logger, request_context):
        ...
        response = yield async_splunk_client.async_get_JWT_validation(self.username, self)
        if response.code == http.OK:
            defer.returnValue(True)
...
def parse_session_token(encryption_context, encrypted_session_token):
    ...
    try:
        ...
        username, password = session_jsn['username'].encode('utf-8'), session_jsn['password'].encode('utf-8')
...
@defer.inlineCallbacks
def get_splunk_cookie(request_context, async_splunk_client, username, password):
    """
    Fetches splunk session cookie
    """

    response = yield async_splunk_client.async_get_splunk_cookie(username, password)
    if response.code == http.OK:
        jsn = yield response.json()
        defer.returnValue(jsn[SESSION_KEY])
...
@defer.inlineCallbacks
def parse_run_as_credentials(encryption_context, system_auth_header, async_client_factory, run_as_credentials):
    """
    :param encryption_context: An encryption context with encryption public and private keys
    ...
    signature_verified = yield verify_run_as_token_signature(encryption_context,
    ...
    defer.returnValue(parse_session_token(encryption_context, session_token_with_expiry.sessionToken))
...
@defer.inlineCallbacks
def verify_run_as_token_signature(encryption_context,
                                  async_client_factory,
                                  system_auth_header,
                                  device_id,
                                  encrypted_session_token_with_expiry_bytes,
                                  signature):
    keys = yield public_keys_for_device(device_id, system_auth_header, async_client_factory.kvstore_client())
    ...
    defer.returnValue(signature_verified)
...
@defer.inlineCallbacks
def process_complete_device_registration_request(request_context,
                                                 client_single_request,
                                                 server_single_response,
                                                 async_client_factory):
    """
    ...
    registration_query_response = yield async_splunk_client.async_get_registration_query(request_context.auth_header,
                                                                                         auth_code,
                                                                                         device_name)
    if registration_query_response.code != http.OK:
        ...
        elif registration_query_response.code == http.NOT_FOUND:
            response_text = yield registration_query_response.text()
        ...
        response_json = yield registration_query_response.json()
        if response_json:
            ...
            if response_confirmation_code != confirmation_code:
                ...
                registration_confirmation_response = \
                    yield async_splunk_client.async_post_registration_confirmation(request_context.auth_header,
                                                                                   auth_code,
                                                                                   temporary_key,
                                                                                   device_name)
