...
def to_dashboard_description(json_object,
                             is_ar=False,
    ...
    if json_object is not None and isinstance(json_object, dict):
        ...
        if content is not None:
            ...
            if 'version' in content and parse_version(str(content.get('version'))) > parse_version("1") \
                ...
                definition = yield to_dashboard_definition(request_context=request_context,
                                                           app_name=app_name,
                                                           root=root_element,
    ...
    defer.returnValue(DashboardDescription())


@defer.inlineCallbacks
def to_minimal_dashboard_description(json_object,
                                     request_context=None,
                                     async_splunk_client=None):
    """
    Parse a dashboard json object and return a DashboardDescription
