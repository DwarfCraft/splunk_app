...
def authenticate_code(auth_code, encryption_context, resolve_app_name, config=SplunkConfig()):
    """
    ...
    raw_response = submit_auth_code(auth_code, encryption_context, config)
    ...
    app_name = resolve_app_name(sb_response_proto.payload.appId)
...
def request_code(device_info, encryption_context, config=SplunkConfig()):
    ...
    r = make_device_authentication_request(device_info, encryption_context, config)

    return parse_device_authentication_response(r)

def fetch_server_credentials(auth_code, encryption_context, num_retries=10, config=SplunkConfig()):
    ...
    for i in range(num_retries):
        r = make_authentication_result_request(auth_code, encryption_context, config)
        try:
